...
// ИЗМЕНЕНИЕ: функция теперь принимает два целевых значения
void testAlgorithmForCase(int size, const char* case_name, 
    ...
    // Вывод результатов
    cout << "Алгоритм\t\t\tВремя (мкС)\tСравн.1\tСравн.2\tРезультат" << endl;
    cout << "----------------------------------------------------------------" << endl;
    for (int i = 0; i < 4; i++) {
...
// ИЗМЕНЕНИЕ: создаем разные целевые значения для отсортированных и несортированных массивов
void testAlgorithmsForSize(int size) {
    ...
    // Три тестовых случая с РАЗНЫМИ целевыми значениями:
    ...
    // 3. Ключ отсутствует (одинаков для всех алгоритмов)
...
// ИЗМЕНЕНИЕ: обновляем generateSummaryTable для использования правильных целевых значений
void generateSummaryTable() {
    ...
    for (int case_idx = 0; case_idx < 3; case_idx++) {
        ...
        for (int size_idx = 0; size_idx < num_sizes; size_idx++) {
            ...
            // Измеряем время для каждого алгоритма с правильными целевыми значениями
            copyArray(unsorted_arr, temp_arr, size);
            auto begin = chrono::steady_clock::now();
