...
void testAlgorithmForCase(int size, const char* case_name, int target, int* unsorted_arr, int* sorted_arr) {
    cout << "\n--- " << case_name << " ---" << endl;
    cout << "Целевое значение: " << target << endl;
    ...
    // Тест 1: Better Linear Search
    copyArray(unsorted_arr, temp_arr, size);
    auto begin = chrono::steady_clock::now();
    results[0] = Better_Linear_Search(temp_arr, size, target, counters1[0], counters2[0]);
    auto end = chrono::steady_clock::now();
    times[0] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

    // Тест 2: Sentinel Linear Search
    copyArray(unsorted_arr, temp_arr, size);
    begin = chrono::steady_clock::now();
    results[1] = Sentinel_Linear_Search(temp_arr, size, target, counters1[1], counters2[1]);
    end = chrono::steady_clock::now();
    times[1] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

    // Тест 3: Ordered Array Search
    copyArray(sorted_arr, temp_arr, size);
    begin = chrono::steady_clock::now();
    results[2] = Ordered_Array_Search(temp_arr, size, target, counters1[2], counters2[2]);
    end = chrono::steady_clock::now();
    times[2] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

    // Тест 4: Binary Search
    copyArray(sorted_arr, temp_arr, size);
    begin = chrono::steady_clock::now();
    results[3] = Binary_Search(temp_arr, size, target, counters1[3], counters2[3]);
    end = chrono::steady_clock::now();
    times[3] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

    // Вывод результатов
    cout << "Алгоритм\t\t\tВремя (мкС)\tcounter1\tcounter2\tРезультат" << endl;
    cout << "----------------------------------------------------------------" << endl;
    for (int i = 0; i < 4; i++) {
...
void testAlgorithmsForSize(int size) {
    ...
    int target_begin = unsorted_arr[0]; // Берем первый элемент несортированного
    testAlgorithmForCase(size, "КЛЮЧ В НАЧАЛЕ МАССИВА", target_begin, unsorted_arr, sorted_arr);

    // 2. Ключ в середине массива
    int target_middle = unsorted_arr[size / 2]; // Берем средний элемент
    testAlgorithmForCase(size, "КЛЮЧ В СЕРЕДИНЕ МАССИВА", target_middle, unsorted_arr, sorted_arr);

    // 3. Ключ отсутствует (заведомо нет в массиве)
    int target_missing = -1; // Гарантированно отсутствует
    testAlgorithmForCase(size, "КЛЮЧ ОТСУТСТВУЕТ", target_missing, unsorted_arr, sorted_arr);
...
void generateSummaryTable() {
    cout << "\n" << string(80, '=') << endl;
    cout << "СВОДНАЯ ТАБЛИЦА ПРОИЗВОДИТЕЛЬНОСТИ (время в микросекундах)" << endl;
    cout << string(80, '=') << endl;

    int sizes[] = { 50000, 80000, 120000, 180000, 240000, 300000 };
    int num_sizes = sizeof(sizes) / sizeof(sizes[0]);
    const char* cases[] = { "Начало", "Середина", "Отсутствует" };
    const char* algorithms[] = { "BLS", "Sentinel", "Ordered", "Binary" };

    // Для каждого случая создаем таблицу
    for (int case_idx = 0; case_idx < 3; case_idx++) {
        ...
        for (int size_idx = 0; size_idx < num_sizes; size_idx++) {
            ...
            // Определяем целевое значение для каждого случая
            int target;
            switch (case_idx) {
            case 0: target = unsorted_arr[0]; break; // Начало
            case 1: target = unsorted_arr[size / 2]; break; // Середина
            case 2: target = -1; break; // Отсутствует
            }

            long long times[4] = { 0 };
            int counter1_dummy, counter2_dummy; // временные переменные для счетчиков

            // Измеряем время для каждого алгоритма
            copyArray(unsorted_arr, temp_arr, size);
            auto begin = chrono::steady_clock::now();
            Better_Linear_Search(temp_arr, size, target, counter1_dummy, counter2_dummy);
            auto end = chrono::steady_clock::now();
            times[0] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

            copyArray(unsorted_arr, temp_arr, size);
            begin = chrono::steady_clock::now();
            Sentinel_Linear_Search(temp_arr, size, target, counter1_dummy, counter2_dummy);
            end = chrono::steady_clock::now();
            times[1] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

            copyArray(sorted_arr, temp_arr, size);
            begin = chrono::steady_clock::now();
            Ordered_Array_Search(temp_arr, size, target, counter1_dummy, counter2_dummy);
            end = chrono::steady_clock::now();
            times[2] = chrono::duration_cast<chrono::microseconds>(end - begin).count();

            copyArray(sorted_arr, temp_arr, size);
            begin = chrono::steady_clock::now();
            Binary_Search(temp_arr, size, target, counter1_dummy, counter2_dummy);
            end = chrono::steady_clock::now();
            times[3] = chrono::duration_cast<chrono::microseconds>(end - begin).count();
